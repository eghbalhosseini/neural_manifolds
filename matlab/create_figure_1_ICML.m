...
%% figure 1.B
ax=axes('position',[.04,.65,.25,.25*xy_ratio]);
cm=inferno(256);
...
% 2nd 
ax2=axes('position',[.32,.765,.15,.15*xy_ratio]);
...
% 3rd
%ax3=axes('position',[.32,.65,.15,.15*xy_ratio]);
...
% 4th 
ax1=axes('position',[.48,.88,.15,.15*xy_ratio]);
mymap = [0 0 0; 1 .3 .3; .3 .3 1];
...
% 5th 
ax2=axes('position',[.48,.765,.15,.15*xy_ratio]);
...
% 6th
ax3=axes('position',[.48,.65,.15,.15*xy_ratio]);
...
ax6=axes('position',[.71,.7,.25,.28]);
hold on;
beta_res=arrayfun(@(x) plot(difference(:,x),beta_vals,'color',line_cols(x,:),...
    'displayname',sprintf('hierarchy %d',x)),1:size(difference,2));

arrayfun(@(x) set(beta_res(x),'LineWidth',2),1:length(beta_res))
set(gca,'yscale','log');

ax6.YLim=[beta_vals(1),beta_vals(end)];
ax6.Box='off';
ax6.XAxis.FontSize=12;
ax6.XAxis.LineWidth=1;
ax6.YAxis.FontSize=12;
ax6.YAxis.LineWidth=1;
legend('position',[.78,.72,.1,.1])
ax6.Legend.FontSize=12;
xlabel('$\bf{\alpha}-\bf{\gamma}$','fontsize',14,'interpreter','latex')
ylabel('$\bf{\beta}$','fontsize',14,'interpreter','latex','rotation',0)
